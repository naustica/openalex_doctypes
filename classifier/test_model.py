...
if __name__ == '__main__':
    ...
    model = DocumentTypeClassifierModel(model_path='model.pkl')

    print('OpenAlex API')
    r = model.make_api_call(url='https://api.openalex.org/w4405561029')

    print(f'Is research: {r.get("label")}')
    print(f'Confidence: {r.get("proba")}')

    print('###############')

    print('Local')
    r = model.get_prediction(
        source_type='journal', # "type":"journal"
        item_type='article', # "type":"article"
        author_count=7, # "authorships":[...]
        has_license=True, # "primary_location": {"license":"cc-by"}
        is_referenced_by_count=54, # "cited_by_count":54
        references_count=29, # "referenced_works_count":29
        has_funder=True, # "funders": [...]
        page_count=18, # "biblio":{"first_page":"2475","last_page":"2492"}
        has_abstract=False, # "abstract_inverted_index":null
        title_word_length=12, # "title":"..."
        inst_count=7, # "institutions_distinct_count":7
        has_oa_url=False, # "open_access":{"oa_url":"..."}
        is_paratext=False, # "is_paratext":false
        issue='4' # "biblio":{"issue":"4"}
    )
